    ...
    while (true) {
        ...
        case 4:
            if (soluongSV > 0) {
                ...
                char strTen[30];
                cout << "\nNhap ten de tim kiem: "; fflush(stdin); gets_s(strTen);
...
void xeploai(SV& sv) {
    if (sv.diemTB >= 8) strcpy_s(sv.hocluc, "Gioi");
    else if (sv.diemTB >= 6.5) strcpy_s(sv.hocluc, "Kha");
    else if (sv.diemTB >= 5) strcpy_s(sv.hocluc, "Trung binh");
    else strcpy_s(sv.hocluc, "Yeu");
}

void nhapThongTinSV(SV& sv, int id) {
    cout << "\n Nhap ten: "; fflush(stdin); gets_s(sv.ten);
    cout << " Nhap gioi tinh: "; gets_s(sv.gioiTinh);
...
void timKiemTheoTen(SV a[], char ten[], int n) {
    ...
    int found = 0;
    for (int i = 0; i < n; i++) {
        strcpy_s(tenSV, a[i].ten);
        if (_strstr(_strupr(tenSV), _strupr(ten))) {
...
void sapXepTheoTen(SV a[], int n) {
    ...
    char tenSV2[30];
    for (int i = 0; i < n; i++) {
        strcpy_s(tenSV1, a[i].ten);
        for (int j = i + 1; j < n; j++) {
            strcpy_s(tenSV2, a[j].ten);
            if (strcmp( _strupr_s(tenSV1), _strupr_s(tenSV2)) > 0) {
                tmp = a[i];
                a[i] = a[j];
                a[j] = tmp;
            }
...
int docFile(SV a[], char fileName[]) {
    ...
    while (fscanf_s(fp, "%5d%30s%5s%5d%10f%10f%10f%10f%10s\n", &a[i].id, &a[i].ten,
