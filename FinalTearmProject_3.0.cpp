...
void ghiFile(sNode* head, char fileName[]) {
	...
	cusOrder x;
	int i = 1;
	fprintf(fp, "========ALL BILL TODAY========\n");
	while (!isEmpty()) {
		P = bpop();
		fprintf(fp, "==============%d===============\n", i);
		fprintf(fp, "Ten khach hang: %s\n", P.cusName);
		for (int j = 0; j < P.no; j++) {
			fprintf(fp, "Ten mon: %s\t So luong: %d\n", P.proName[j],P.quantity[j]);
		}
		fprintf(fp, "Tong thanh toan: %d\n", P.pay);
		i++;
...
void buy()
{
	...
	else
	{
		...
		cusOrder infoCus;
		string customer;
		cout << "\n\t\t Enter Your Name :"; cin.ignore(32767, '\n'); getline(cin, customer);
		enqueue(customer);
		system("cls");
		cout << "\n\n\n\n\t\t\tYou have bought : ";
		for (int j = 0; j < no; j++)
...
void administator()
{
	...
	do {
		...
		cout << "\t\t     Enter 7 for Write Bill list to file" << endl;
		cout << "\t\t     Enter 8 for Sort by price" << endl;
		cout << "\t\t     Enter 9 for Customer bill" << endl;
...
int main()
{
	node* p1 = new node(01, "Capuchino", 20000, 50);
	node* p2 = new node(02, "Milk Tea", 30000, 50);
	node* p3 = new node(03, "Hot Cafe", 25000, 80);
	addLast(p1);
	addLast(p2);
	addLast(p3);
